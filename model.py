...
from typing import List, NamedTuple, Tuple, Optional, Union, TYPE_CHECKING

if TYPE_CHECKING:
    from alignment import IntDensePairFusionBank

from dataset import NUM_HOLIDAY_TYPES, NUM_PROMO_IDS
...
class ModelInput(NamedTuple):
    ...
    request_hour: torch.Tensor       # (B,) int64
    request_dow: torch.Tensor        # (B,) int64
    request_dom: torch.Tensor        # (B,) int64
    request_weekend: torch.Tensor    # (B,) int64
    request_holiday_type: torch.Tensor  # (B,) int64, 0..2
    request_promo_id: torch.Tensor      # (B,) int64, 0..8
...
class GroupNSTokenizer(nn.Module):
    ...
    def __init__(self, feature_specs: List[Tuple[int, int, int]],
                 ...
                 pair_fusion_bank: Optional["IntDensePairFusionBank"] = None) -> None:
        super().__init__()
        self.feature_specs = feature_specs
        self.groups = groups
        self.emb_dim = emb_dim
        self.emb_skip_threshold = emb_skip_threshold
        self.pair_fusion_bank = pair_fusion_bank
    ...
    def forward(
        ...
        Args:
            ...
            dense_feats: optional (B, user_dense_dim) for int-dense fusion.
        ...
        for group, proj in zip(self.groups, self.group_projs):
            ...
            for fid_idx in group:
                ...
                if self.pair_fusion_bank is not None and dense_feats is not None:
                    fid_emb = self.pair_fusion_bank.fuse_fid_embedding(
                        fid_idx, fid_emb, dense_feats,
                    )
...
class RankMixerNSTokenizer(nn.Module):
    ...
    def __init__(
        ...
        num_ns_tokens: int,
        emb_skip_threshold: int = 0,
        pair_fusion_bank: Optional["IntDensePairFusionBank"] = None,
    ) -> None:
...
class RequestCalendarNSTokenizer(nn.Module):
    """Fuse multi-view request calendar embeddings into one NS token."""

    def __init__(self, d_model: int, use_social_calendar: bool = False) -> None:
        super().__init__()
        self.use_social_calendar = use_social_calendar
        self.emb_hour = nn.Embedding(24, d_model)
        self.emb_dow = nn.Embedding(7, d_model)
        self.emb_dom = nn.Embedding(31, d_model)
        self.emb_weekend = nn.Embedding(2, d_model)
        if use_social_calendar:
            self.emb_holiday_type = nn.Embedding(NUM_HOLIDAY_TYPES, d_model)
            self.emb_promo_id = nn.Embedding(NUM_PROMO_IDS, d_model)
        self.norm = nn.LayerNorm(d_model)
    ...
    def forward(
        ...
        x = (
            ...
            + self.emb_dow(dow)
            + self.emb_dom(dom)
            + self.emb_weekend(weekend)
        )
        if self.use_social_calendar:
            x = x + self.emb_holiday_type(holiday_type) + self.emb_promo_id(promo_id)
        x = self.norm(x)
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        use_request_time_ns: bool = False,
        use_request_social_calendar: bool = False,
        use_int_dense_alignment: bool = False,
        fid_alignments: Optional[list] = None,
        dense_residual_slices: Optional[List[Tuple[int, int]]] = None,
        ...
        self.use_int_dense_alignment = use_int_dense_alignment
        self._dense_residual_slices = dense_residual_slices or []
        self.residual_dense_dim = residual_dense_dim
        self.has_user_dense_input = user_dense_dim > 0

        pair_fusion_bank: Optional["IntDensePairFusionBank"] = None
        if use_int_dense_alignment:
            from alignment import IntDensePairFusionBank
            if not fid_alignments:
                raise ValueError("use_int_dense_alignment requires fid_alignments")
            pair_fusion_bank = IntDensePairFusionBank(fid_alignments, embed_dim=emb_dim)
        ...
        if ns_tokenizer_type == 'group':
            ...
            self.user_ns_tokenizer = GroupNSTokenizer(
                ...
                pair_fusion_bank=pair_fusion_bank,
            )
        ...
        if use_int_dense_alignment:
            self.has_user_dense_ns = residual_dense_dim > 0
            if self.has_user_dense_ns:
                self.user_dense_proj = nn.Sequential(
                    nn.Linear(residual_dense_dim, d_model),
                    nn.LayerNorm(d_model),
                )
            else:
                self.user_dense_proj = None
        else:
            self.pair_fusion_bank = None
            self.has_user_dense_ns = user_dense_dim > 0
            if self.has_user_dense_ns:
                self.user_dense_proj = nn.Sequential(
        ...
        if use_request_social_calendar and not use_request_time_ns:
            raise ValueError("use_request_social_calendar requires use_request_time_ns=True")
        self.use_request_time_ns = use_request_time_ns
        self.use_request_social_calendar = use_request_social_calendar
        if use_request_time_ns:
            self.request_calendar_ns = RequestCalendarNSTokenizer(
                d_model, use_social_calendar=use_request_social_calendar
            )
        else:
            self.request_calendar_ns = None

        # Total NS token count
        self.num_ns = (
            num_user_ns
            + (1 if self.has_user_dense_ns else 0)
            + num_item_ns
    ...
    def _gather_user_dense_residual(self, user_dense_feats: torch.Tensor) -> torch.Tensor:
        from alignment import gather_dense_slice
        return gather_dense_slice(user_dense_feats, self._dense_residual_slices)

    def forward(self, inputs: ModelInput) -> torch.Tensor:
        """Runs the forward pass of the PCVRHyFormer model."""
        # 1. NS tokens: grouped projection
        user_dense_for_tok = (
            inputs.user_dense_feats if self.use_int_dense_alignment else None
        )
        user_ns = self.user_ns_tokenizer(inputs.user_int_feats, user_dense_for_tok)
        item_ns = self.item_ns_tokenizer(inputs.item_int_feats)

        ns_parts = [user_ns]
        if self.has_user_dense_ns:
            if self.use_int_dense_alignment:
                dense_in = self._gather_user_dense_residual(inputs.user_dense_feats)
            else:
                dense_in = inputs.user_dense_feats
            user_dense_tok = F.silu(self.user_dense_proj(dense_in)).unsqueeze(1)
            ns_parts.append(user_dense_tok)
        ns_parts.append(item_ns)
        if self.has_item_dense:
            item_dense_tok = F.silu(self.item_dense_proj(inputs.item_dense_feats)).unsqueeze(1)  # (B, 1, D)
            ns_parts.append(item_dense_tok)
        if self.use_request_time_ns:
            cal_tok = self.request_calendar_ns(
                inputs.request_hour,
                inputs.request_dow,
                inputs.request_dom,
                inputs.request_weekend,
                inputs.request_holiday_type,
                inputs.request_promo_id,
            )
            ns_parts.append(cal_tok)
